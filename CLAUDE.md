...
- **GemmaImagePrompt** (`Gemma Image Prompt`) — EXPERIMENTAL/test node. Uses a vision-capable Gemma4 encoder (from `CLIPLoader` type `gemma4`) to look at an input image and write a text-to-image prompt that recreates a visually similar image, honoring free-form modification instructions. Inputs: `clip` (CLIP, required), `image` (IMAGE, optional — fed to the multimodal tokenizer via `clip.tokenize(..., image=image)`), `instruction` (multiline), `output_format` (combo Natural language/Danbooru tags), `target_model` (combo FLUX/SDXL — FLUX leaves negative empty, SDXL produces a negative), `detail_mode` (combo Keep as instructed/Expand detail), `max_length`, `unload_after`, `prompt_mode` (combo Generate (recreate image)/Edit instruction (change description)/Video description (LTXV) — Edit, added 2026-06-08, makes Gemma emit a "change X into Y" before→after editing instruction for image-editing models like Qwen-Image-Edit instead of a recreation prompt, and leaves negative empty; **Video description (LTXV)**, added 2026-07-19, writes an LTX-2/LTXV text-to-video prompt via `_build_video_request()` — one flowing paragraph, present tense, explicit camera moves, chronological literal motion, negative left empty), `video` (**IMAGE,VIDEO** union, optional — a video frame batch OR a ComfyUI `VIDEO` object; added 2026-07-19, VIDEO accepted 2026-07-29), `max_frames` (INT, default 8 — frame cap; **change-based** selection since 2026-08-18, evenly sampled before that; added 2026-07-19). Outputs `positive` / `negative`. **Video path**: `_generate()` sends the frame batch through the tokenizer's native `video=` kwarg when the tokenizer declares one (Gemma4 — detected via `_tokenizer_accepts()` inspecting `clip.tokenizer.tokenize_with_weights`'s signature), else falls back to `image=` (Qwen3-VL swallows unknown kwargs into `**kwargs`, so a plain TypeError fallback would silently drop the frames — hence the signature check). Video takes precedence over a still image when both are wired; `_extract_video()` first normalizes the input (a `VIDEO`/`VideoInput` object → `get_components().images` + `.frame_rate`; an IMAGE batch passes through with fps unknown), then `_select_frame_indices()` picks at most `max_frames` frames **by how much the picture changed** (first + last + biggest changes) and `_frame_note()` states their timestamps in the request. The request prompt is assembled by `_build_request()` from the settings and commands "output ONLY the prompt" in a `POSITIVE:`/`NEGATIVE:` form that `_parse_pos_neg()` splits (falls back to all-positive if labels missing; normalizes `(empty)`/`none` negatives to `""`). With no image AND no instruction, returns `("", "")`. Runs during graph execution like GemmaTranslate; results pushed to two display fields via the `ui` payload. Own frontend (`web/gemma_image_prompt.js`, native widgets only). Added 2026-06-08. **Qwen3-VL also works** (ComfyUI v0.26.0+ added Qwen VL support): a vision-capable Qwen3-VL loaded via `CLIPLoader` can be wired into `clip` in place of Gemma4 with NO code changes (confirmed working 2026-06-29) — the `clip.tokenize(..., image=image)` / `clip.generate` / `clip.decode` pipeline is model-agnostic
...
### Recent Changes (September 8, 2026)
- ✅ **`GemmaImagePrompt`: errors now stop the job instead of being written into the output** (`nodes.py` + tests). Previously `execute()` caught every exception from frame extraction / request building / generation and returned `[Gemma Image Prompt error] <Type>: <msg>` as the `positive` output, so a failed run looked like a *successful* run whose prompt happened to be an error string — the error text then flowed downstream into whatever consumed `positive` (a KSampler conditioning, a save-text node) and ComfyUI's queue reported success. The user asked for the normal behavior: fail loudly
  - The `except Exception` around the frame-selection / `_build_request` / `_generate` / `_parse_pos_neg` block now **re-raises** (`raise`) after honoring `unload_after`, so the exception reaches ComfyUI's executor, the job stops, and the real traceback is shown
  - **`unload_after` still runs on the error path** — VRAM is released on the way out, otherwise a crashed run would strand the whole model cache. The unload body was extracted into a `_unload_models()` staticmethod (the same best-effort `unload_all_models()` + `soft_empty_cache()` in a try/except) and is called from both the error path and the success path
  - **Unchanged**: the "no image AND no video AND no instruction" short-circuit still returns `("", "")` — that is not an error. `GemmaTranslate` still catches its generation errors into `[Gemma Translate error] …` (deliberately left alone — the user asked about this node only)
  - `test_generation_error_is_caught` → `test_generation_error_propagates` (`assertRaises(RuntimeError)`). README JA+EN gained a note that errors stop the job
- ✅ **`GemmaImagePrompt`: the raised error now names the stage AND the likely cause** (same-day follow-up, `nodes.py` + tests). Raising bare is honest but useless on its own — the common failures surface as a `TypeError` from deep inside a tokenizer, or an `AttributeError` on `clip.generate`, neither of which tells the user that they wired the wrong `CLIPLoader` type. The message is now three lines: `[Gemma Image Prompt] 失敗しました / failed（段階 / stage: …）` + `原因の可能性 / likely cause: …` + `元のエラー / original error: <Type>: <msg>`, bilingual JA+EN like the `_PPFTranslateError` messages
  - **Stage tracking**: `execute()` keeps a `stage` local (`"video"` → `"request"` → `"generate"` → `"parse"`) reassigned before each block inside the existing try; the `except` calls `cls._fail(e, stage)` (which always raises `RuntimeError(...) from e`, so the original traceback is chained and visible). `_STAGE_LABELS` maps the key to a bilingual label
  - **`_error_hint(exc, stage)`** picks the cause by inspecting the exception, in order: `"unexpected keyword argument"` + image/video → **not a vision model**; `AttributeError` mentioning generate/tokenize/decode → **CLIP has no text generation** (wrong CLIPLoader type / ComfyUI < v0.21); `"out of memory"` / `OutOfMemoryError` → **VRAM**; tensor-shape phrases (`size of tensor`, `mat1 and mat2`, `must match`, …) → **model/input mismatch**; else `stage == "video"` → **video decode** (wire core `Load Video`'s VIDEO or a VHS IMAGE batch; trim long clips); else a generic 3-point checklist (CLIPLoader type / ComfyUI too old / VRAM). Hints are class constants (`_HINT_NO_VISION`, `_HINT_NOT_GENERATIVE`, `_HINT_OOM`, `_HINT_SHAPE`, `_HINT_VIDEO`, `_HINT_GENERIC`) so tests and future callers share the exact wording
  - **`_check_clip(clip, needs_vision)` runs BEFORE the try** and fails fast on the two mistakes that are knowable without running anything: (1) `clip` missing any of `tokenize`/`generate`/`decode` → names which are missing + the wrong-CLIPLoader-type hint (this is the plain SD/SDXL CLIP case); (2) a vision input is wired but the tokenizer declares no `image`/`images`/`video` parameter → the not-a-vision-model hint. The vision check is deliberately permissive: `_tokenizer_takes_kwargs()` returns True for a `**kwargs` catch-all **and when the signature can't be read at all**, so a model that would actually work (Qwen3-VL) is never blocked by our guess
  - **Unchanged**: `unload_after` still runs on the error path; the "nothing wired" short-circuit still returns `("", "")`; `GemmaTranslate` still catches its errors into the output (out of scope — the user asked about this node)
  - Added 6 tests (stage+cause+original error present, CLIP without `generate`, text-only model + image, OOM, video decode, `unexpected keyword argument 'image'` recovered at generation time) → **99 tests pass** via `python -m unittest discover -s tests`

### Recent Changes (August 18, 2026)
- ✅ **`GemmaImagePrompt`: video frames are now picked by CHANGE, not by even spacing** (`nodes.py` + tests; frontend unchanged). Ported from **`D:\_CODE\ComfyUI-LLM-Widget`**'s video sampling at the user's request. The old `_sample_frames` took evenly-spaced indices, so a clip that holds still and then does one thing (a cut, a gesture, a camera move) sent the same held frame several times over and missed the one moment worth describing — the worse case being Qwen3-VL, where every frame costs a full still's worth of context
  - **Algorithm** (mirrors the reference repo): candidates at roughly **1 fps** along the clip (`_candidate_indices`, last frame always included, capped at `_MAX_CANDIDATES = 180`), each scored by **mean absolute difference from the previous candidate** on a coarse strided view (`_change_scores`, `_SCORE_SIDE = 48` — what matters is that the composition moved, not that the encoder's noise did), then `_select_by_change` keeps the **first and last** candidate (a clip is judged by where it starts and ends) and spends the rest of the budget on the biggest changes
  - **Deviation from the reference — candidate density**: the reference passes `minimum = limit`, which for a typical short clip (5 s @ 24 fps, budget 8) collapses the candidate set to exactly the budget, so the change scoring has nothing to choose between and the result degrades to even spacing — i.e. the feature would have done nothing for exactly the clips this node is used on. `_select_frame_indices` therefore asks for `max_frames * _CANDIDATE_FACTOR` (4×) candidates
  - **Deviation from the reference — tie-breaking**: equal scores are broken by **distance from what is already chosen**, not by lowest index. Real footage never ties (sensor noise alone moves the number), but a synthetic or perfectly still clip otherwise spends its whole budget on the first few candidates, which reads as a long dwell that never happened. Verified: a static 120-frame clip now yields `[0, 15, 30, 45, 60, 75, 90, 119]`; a 120-frame clip cut at frame 90 yields `[0, 21, 33, 45, 66, 90, 105, 119]` (the cut is kept AND the clip is covered)
  - **Frame timestamps are now stated in the request** (`_frame_note`, appended by `_build_request` / `_build_video_request` when a video is wired): "The video is provided as N frames sampled from the clip at 0.0s / 1.2s / … The spacing is uneven — more frames are taken where the picture changes — so read the timestamps rather than assuming a constant interval." **Required**, not decoration: an uneven sample read as a constant interval makes the model describe motion at the wrong speed (the reference repo hit the same problem). Timestamps need a frame rate, so a bare IMAGE batch (VHS-style loader) gets the same note **without** the stamps
  - **`_extract_video_frames()` → `_extract_video()`**, returning `(frames, fps)`: the frame rate is only reachable through `VideoComponents.frame_rate`, and `get_components()` must not be called twice (it decodes the whole clip). `_extract_video_frames()` is kept as a frames-only wrapper. `execute()` now samples **before** building the request (the request has to know the frame count/times), so `_build_request` moved inside the existing try/except — a build error stops the job like a generation error (**as of 2026-09-08**; it used to surface as `[Gemma Image Prompt error] …` in `positive`)
  - **New helpers**: `_candidate_indices` / `_change_scores` / `_select_by_change` / `_select_frame_indices` / `_take_frames` / `_frame_times` / `_frame_note`. `_sample_frames(video, max_frames, fps=None)` survives as a thin wrapper (select + take) and still returns the input unchanged for a non-indexable sentinel. `_take_frames` returns the batch itself when nothing was dropped (avoids a pointless tensor copy). `_change_scores` returns `None` when the batch doesn't support tensor ops, and the caller then thins the candidates evenly — so the node never fails because of frame selection
  - **Tests**: new `_PixelBatch` double (one scalar brightness per frame; supports the list/slice/tuple indexing, `.float()`, `.abs()`, `.mean(dim=…)` that `_change_scores` performs) so the change path is exercised **without torch**; `_FakeVideoObject` moved to module level and given a `frame_rate`. Added 8 tests (cut is picked, static clip stays spread, `_select_by_change` keeps both ends, candidate coverage/short-clip density, fps extraction, `_frame_times`, request states timestamps, request omits them without fps) → **93 tests pass** (was 85), via `python -m unittest discover -s tests`
  - **Cost**: selection is pure Python over at most 180 candidates — worst case (`max_frames=64`) measured at ~16 ms per execution. The scoring pass reads `max_frames*4` frames of the batch at a ~48 px stride
  - **Unchanged**: `_extract_video()` still materializes the WHOLE video via `get_components()` before selection (the API has no lazy frame-range read), so long clips still want a `Trim Video` in front. Verified by the test suite, not yet by a live ComfyUI run

### Recent Changes (August 12, 2026)
- ✅ **`NodeValueTemplate`: token picker now inserts at the caret (3rd attempt — the previous two fixed the wrong thing)** (`web/node_value_template.js` — frontend only). Symptom history: with existing text in `template`, the picker inserted at "neither the caret nor the end" → after the value-snapshot fix below it inserted **always at the end**, i.e. the recorded caret was never usable. That second symptom is what identified the real cause
  - **Real root cause — we were watching the wrong `<textarea>`.** Caret tracking was attached to `widget.inputEl || widget.element`, but under the Vue renderer the box the user actually types in is the textarea **mounted inside the node's DOM root** (`.lg-node[data-node-id="<id>"]`), which is NOT necessarily that element. So no tracked event ever fired, `document.activeElement === ta` was never true (the focused element was the *other* textarea), and the caret was either frozen at a stale offset (→ "weird position", pre-Aug-12) or absent (→ "always at the end", after the element-identity/value guards were added)
  - **Fix — resolve the textarea through the DOM, and record which element the caret belongs to**:
    - `nvtNodeForTextArea(ta)` — reverse lookup, textarea → node: `ta.closest('.lg-node[data-node-id]')` → `getNodeById` (and bail immediately if that node isn't a `NodeValueTemplate`, which keeps the global listener cheap), falling back to scanning `_nodes` for one whose `getTemplateTextArea()` IS this element (the Classic-mode answer). Cached on `ta._nvtNode`
    - `findTemplateTextArea(node)` — forward lookup used when there is no record: prefers the single `<textarea>` inside the node's DOM root, falls back to the widget's own element
    - `installGlobalCaretTracker()` — **one** document-level capture listener set (`selectionchange`/`keyup`/`mouseup`/`input`/`focusin`) for ALL nodes, resolving `document.activeElement` through `nvtNodeForTextArea`. Deliberately global-and-single (constant memory, `window.__nvtCaretHooked` guard — same pattern as `index.js`'s wheel/mousemove hooks) instead of per-node listeners, which would need `onRemoved` teardown this file doesn't have. Installed from `setup()` and `onNodeCreated`. Replaced the old per-element `ensureCaretTracker()`, which is gone along with `_nvtCaretEl` / `_nvtOwner` / `_nvtTracked`
    - `captureCaretNow(node)` on the picker button's `pointerdown` (before the button steals focus) records the caret of whatever textarea is focused *if it belongs to this node*
  - **Kept from the earlier attempt** (still needed — programmatic `textarea.value = …` from a Vue re-render / workflow undo / configure fires NO event): the record is `{el, start, end, value}` and `resolveCaret(node)` returns both the element AND the offsets — focused box → live caret; else the record **only if `rec.value === ta.value`**; else keep `start` when `ta.value.slice(0, rec.start)` still matches the recorded prefix (a selection collapses to a caret); else no caret → append at the end. The fallback is always the predictable "末尾", never a random offset
  - **Also**: `insertToken(node, token, refocus = true)` — `doInsert(close)` passes `close`, so an insert that keeps the modal open ends with the textarea **blurred** and consecutive inserts chain off the record instead of re-reading a live caret the renderer may have moved. The insertion prefers `document.execCommand("insertText", …)` (native undo history, browser-managed caret) and verifies `ta.value === next`, falling back to the manual splice + synthetic `input` event. Writes go to the resolved element, so `widget.inputEl` being a different node is no longer a problem. `window.__nvtDebug = true` in the console logs each insert's element/offset for future diagnosis
  - **Verified** by evaluating the module in Node (`vm` + fake `document`/textarea, `execCommand` emulated, both a Classic node and a Nodes-2.0 node whose `widget.inputEl` differs from the mounted textarea): 17 checks — caret insert in both modes, caret straight after typing, live caret beating an old record, selection replacement, stale value → append, prefix-intact → keep caret, never-focused → append, chained inserts, execCommand-unavailable fallback, two nodes keeping separate carets. YARA-clean (no `Function#bind` / dotted socket-method substrings)

### Recent Changes (July 29, 2026)
- ✅ **`GemmaImagePrompt`: the `video` slot now accepts the `VIDEO` type** (`nodes.py` + tests; frontend unchanged): the user reported that a **`Load Video` → `video`** wire would not connect (an IMAGE output connected fine). Root cause: ComfyUI **core**'s `Load Video` outputs `VIDEO` (a `comfy_api` `VideoInput` object), NOT an IMAGE frame batch — only VHS-style loaders output IMAGE — and our slot was declared `("IMAGE", …)`, so LiteGraph refused the link
  - **Fix — union type**: the slot is now `"IMAGE,VIDEO"`. V1 `INPUT_TYPES` declares the comma string directly; the V3 `define_schema` builds `io.MultiType.Input("video", types=[io.Image, io.Video], optional=True)` (whose `get_io_type()` produces the same `"IMAGE,VIDEO"` string), inside a `try/except` that falls back to `io.Image.Input` on older `comfy_api` builds without `MultiType`. Union types are honored on **both** sides: the frontend's connection check splits on `,`, and the backend's `comfy_execution/validation.py:validate_node_input()` accepts an overlap between received/input type lists
  - **New `_extract_video_frames(video)` staticmethod**: normalizes whatever arrives on the slot to a frame batch before `_sample_frames()` — a `VIDEO` object is detected by a callable `get_components` attribute (duck-typed, so `nodes.py` still imports with no ComfyUI present) and unwrapped via `get_components().images`; a plain IMAGE batch passes through unchanged; `None` stays `None`. Called inside `execute()`'s try/except; a decode failure **raises** and stops the job (**as of 2026-09-08** — it used to surface as `[Gemma Image Prompt error] …` in `positive`)
  - **Caveat**: `VideoInput.get_components()` materializes the WHOLE video into memory before `max_frames` sampling happens (the API has no lazy frame-range read). For long clips, pre-trim with ComfyUI's `Trim Video` node or feed frames from a VHS loader instead
  - Added 3 tests (VIDEO object → frames extracted and routed through `video=`, the slot's declared type is the IMAGE/VIDEO union, IMAGE/`None` passthrough) → **85 tests pass** (was 82). README JA+EN + node summary updated. NOTE: the suite runs under `python -m unittest discover -s tests` — `python -m pytest` errors out during collection for an unrelated pre-existing reason

### Recent Changes (July 19, 2026)
- ✅ **`GemmaImagePrompt`: video input + LTXV video-description mode** (`nodes.py` + tests; frontend needs NO change — the `video` slot is a native IMAGE input auto-created by ComfyUI): the node can now analyze a video and write a text-to-video prompt for **LTX-2 / LTXV**
  - **New inputs (appended last for `widgets_values`/slot index stability)**: `video` (IMAGE, optional — in ComfyUI a "video" is an IMAGE batch of frames, e.g. from a Load Video node) and `max_frames` (INT, default 8). New `prompt_mode` option **`Video description (LTXV)`** (a third mode alongside Generate / Edit instruction)
  - **Prompt engineering**: `_build_request()` now branches to a new `_build_video_request()` when `prompt_mode` contains "video". It assembles an LTX-2-style request per the official LTX prompting guide — ONE flowing paragraph, 4–8 sentences, present tense, chronological *literal* motion (not abstract), explicit camera language (pans/tracks/pushes in/pulls back/tilts/…), shot type + lighting + color + texture + atmosphere, and **positive-only** (negative left empty). `detail_mode=Expand detail` adds background/secondary motion. Ignores `output_format`/`target_model` (always NL, always empty negative). Works with `video`, with a still `image`, or from `instruction` text alone
  - **Video tokenizer routing** (the subtle part): `_generate()` sends the frame batch through the tokenizer's **native `video=` kwarg** when the tokenizer declares one — **Gemma4** has `tokenize_with_weights(..., image=None, audio=None, video=None, ...)` and subsamples frames at 1fps as a temporal sequence. Detection is `_tokenizer_accepts(clip, "video")` which `inspect.signature`-checks `clip.tokenizer.tokenize_with_weights` for an explicit `video` param. **Why not a TypeError fallback**: Qwen3-VL's tokenizer is `tokenize_with_weights(..., images=[], ..., **kwargs)` — it *swallows* an unknown `video=` into `**kwargs` and silently drops the frames (no TypeError raised), so a naive `try video= / except TypeError: image=` would lose the video on Qwen. When `video=` isn't a real param we send `image=` instead (Qwen splits the batch into per-frame stills; no temporal encoding). Video takes precedence over a still `image` when both are wired
  - **`_sample_frames(video, max_frames)`**: uniformly downsamples the `[frames,H,W,C]` batch to at most `max_frames` (evenly-spaced indices, `video[idx]` list-indexing) before the frames reach the model — caps VRAM/context (critical for Qwen, which stills every frame). Defensive: non-tensor sentinels pass straight through (so tests need no torch). **Superseded 2026-08-18** by change-based selection (see that entry) — `_sample_frames` is now a wrapper over `_select_frame_indices()` + `_take_frames()`
  - **Tests**: `_FakeVisionClip` extended to record `got_video` and expose a `tokenizer.tokenize_with_weights` whose signature declares `video` (so `_tokenizer_accepts` returns True, mirroring Gemma4). Added 6 tests (LTXV request wording, video→`video=` routing, video→`image=` fallback for a no-`video=` tokenizer, LTXV-from-instruction, video-beats-image precedence, `_sample_frames` capping). **82 tests pass** (was 76). README JA+EN + node summary updated
  - **Caveat**: EXPERIMENTAL / model-dependent. Gemma-4-E4B's video understanding is limited; Qwen3-VL has weak temporal info (frames as stills). Confirmed by the test suite, not yet by a live ComfyUI run

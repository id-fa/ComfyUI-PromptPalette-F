...
6. **NodeValueTemplate Node** (`nodes.py` + `web/node_value_template.js`):
   ...
   - **Token picker modal** (`web/node_value_template.js`): a `🔍 ノードの値を挿入…` button (added via `addDOMWidget` so it works in both renderers, like the prompt_tabs tab bar) opens a modal helper for building tokens without typing
     ...
     - **Flow** (confirmed design): two sections feed one shared selection (`state.token`) — `selectRow()` clears highlight across BOTH lists so node-value and date tokens are mutually exclusive. (1) **Node value**: node-title `<select>` (blank first option, nothing pre-chosen) → scrollable list of that node's widget names + current values; picking a node pre-selects its first prop. (2) **Date format**: a list of `%date:FORMAT%` samples (`DATE_SAMPLES`) each with a live `formatDate()` preview. Click a row to select (double-click inserts immediately + keeps modal open); the `挿入` button inserts the selected token at the caret. Date formats are available even with zero referenceable nodes (the no-nodes case shows a hint in the node section but still renders the date section). Avoids dumping every node's widgets at once (heavy with many nodes) — only the chosen node's widgets are expanded
...
- ✅ **`NodeValueTemplate` `%date:FORMAT%` expansion** (same-day follow-up, `web/node_value_template.js`): `resolveTemplate()` now also expands `%date%` / `%date:FORMAT%` like SaveImage's `filename_prefix`. `formatDate(format, dt)` supports `yyyy`/`yy`/`MM`/`M`/`dd`/`d`/`hh`/`h`/`mm`/`m`/`ss`/`s` (case-sensitive), via a left-to-right longest-first scan so `yyyy` beats `yy` and substituted digits aren't re-matched; non-token chars kept literal; bare `%date%` → `yyyy-MM-dd`. Runs frontend-side (browser local time at queue time). README (JA+EN) + CLAUDE.md updated
  - **Picker modal extended with date samples** (same-day): the token picker now has two sections feeding one shared `state.token` (mutually exclusive via `selectRow()` clearing both lists) — a "ノードの値" section (title dropdown + prop list, unchanged) and a "日付フォーマット" section listing `DATE_SAMPLES` (`yyyy-MM-dd`, `yyyy-MM-dd_hh-mm-ss`, `yyyyMMdd`, …) each with a live `formatDate()` preview. Date samples are available even when there are no referenceable nodes

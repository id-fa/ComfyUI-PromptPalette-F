...
- ✅ **`GemmaImagePrompt`: video input + LTXV video-description mode** (`nodes.py` + tests; frontend needs NO change — the `video` slot is a native IMAGE input auto-created by ComfyUI): the node can now analyze a video and write a text-to-video prompt for **LTX-2 / LTXV**
  ...
  - **`_sample_frames(video, max_frames)`**: uniformly downsamples the `[frames,H,W,C]` batch to at most `max_frames` (evenly-spaced indices, `video[idx]` list-indexing) before the frames reach the model — caps VRAM/context (critical for Qwen, which stills every frame). Defensive: non-tensor sentinels pass straight through (so tests need no torch)

...
`googletrans` を使う `Prompt Tabs + Translate` とは違い、こちらは**Gemma4のローカルLLMによる翻訳**を行います。`CLIPLoader`（type: `gemma4`）で読み込んだ Gemma4 モデルを `clip` 入力に接続し、ComfyUI標準の `TextGenerate` ノードと同じ生成処理（`clip.tokenize` → `clip.generate` → `clip.decode`）で翻訳します。外部サービスを使わずローカルで完結します。
...
| 入力 | 型 | 説明 |
| --- | --- | --- |
| `clip` | CLIP | `CLIPLoader`（type: `gemma4`）で読み込んだ Gemma4 エンコーダ（必須） |
...
| 入力 | 型 | 説明 |
| --- | --- | --- |
| `clip` | CLIP | `CLIPLoader`（type: `gemma4`）で読み込んだ vision対応 Gemma4 エンコーダ（必須） |
...
| Input | Type | Description |
| --- | --- | --- |
| `clip` | CLIP | A Gemma4 encoder loaded by a `CLIPLoader` (type: `gemma4`) (required) |
...
| Input | Type | Description |
| --- | --- | --- |
| `clip` | CLIP | A vision-capable Gemma4 encoder loaded by a `CLIPLoader` (type: `gemma4`) (required) |

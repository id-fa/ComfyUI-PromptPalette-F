...
翻訳はボタンを押した時点でその場で実行されます（ワークフロー実行を待つ必要はありません）。`googletrans` を利用するためAPIキーは不要です。`googletrans` が未インストールの環境では、Googleの無料翻訳エンドポイントへ自動的にフォールバックします（こちらもAPIキー不要）。
...
Translation runs **immediately on button click** (you don't have to queue the workflow). It uses `googletrans`, so no API key is needed. When `googletrans` isn't installed, the node automatically falls back to Google's free translate web endpoint (also no API key).
